# Evualation quality of prediction on logistic regression using 
# Indels & SNPs & aggregated by gene datasets
...
# Add gene aggr features

aggr = data.transpose()
aggr['gene'] = temp['gene']
genes = np.unique(temp['gene'])
aggr = aggr.groupby(by='gene').sum().transpose()
aggr[aggr > 1] = 1

data = pd.concat([data, aggr], axis=1)
...
for i in range(1, N_ITERS + 1):
    for j in range(1, N_SPLIT + 1):
        ...
        coef['ext_features'] = [get_name_by_ind(pos_info, int(x), drug) if x not in genes else x for x in columns]
