# Evualation quality of prediction on logistic regression using Indels & SNPs datasets
...
from scipy.sparse import csc_matrix, load_npz, hstack
...
temp = pos_info[(pos_info['act'] == 'ins') | (pos_info['act'] == 'snp') |
        (pos_info['ind2'] == 'del') | (pos_info['act'] == 'del')]
...
for i in range(1, N_ITERS + 1):
    for j in range(1, N_SPLIT + 1):
        ...
        domain_train = load_npz(f"{domain_dir}/{i}/{drug}.gt.domains.3.fold.{j}.train.npz")
        domain_test = load_npz(f"{domain_dir}/{i}/{drug}.gt.domains.3.fold.{j}.test.npz")
        ...
        X_train = hstack([X_train, domain_train])
        X_test = hstack([X_test, domain_test])

        domain_pos_info = pd.read_csv(f"{domain_dir}/{i}/{drug}.gt.pos.domains.3.fold.{j}", sep='\t',
                                        index_col=None, header=None, usecols=[0, 1])
        domain_pos_info.columns = ['gene', 'pos']
        domain_pos_info['ind1'] = ''
        domain_pos_info['ind2'] = ''
        domain_pos_info['act'] = ''

        domain_columns = range(len(pos_info), len(pos_info) + len(domain_pos_info))
        columns = np.concatenate((columns, domain_columns))

        pos_info = pd.concat([pos_info, domain_pos_info])
